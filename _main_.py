...
hello,title = st.columns((1,9), gap="small")
with hello:
...
if file is not None:
    ...
    if error_caught:
        ...
        for i, level in enumerate(levels):
            if level == "Continuous":
                recommendations[data.columns[i]] = ["ANOVA", "SIMPLE LINEAR REGRESSION"]
            elif level == "Binary":
        ...
        if data.empty:
            ...
            st.sidebar.markdown("""---""")
            column = st.sidebar.selectbox("➕ SELECT THE 'y' FIELD (dependent variable):", list(recommendations.keys()))
            st.write("\n")
            test = st.sidebar.selectbox("➕ SELECT AN INFERENTIAL TEST (determined by the selected y field above):", recommendations[column])
        ...
        if data.empty:
            ...
            if test == "SIMPLE LINEAR REGRESSION":
                linear_regression_test.linear_regression(data,file,column)
            elif test == "T-TEST (PAIRED)":
                anova_test.t_test_paired(data,file,column)
            elif test == "ANOVA":
                anova_test.anova(data,file,column)
            elif test == "CHI-SQUARE TEST":
                chi_square_test.chi_square(data,file,column)
            elif test == "LOGISTIC REGRESSION":
                logistic_regression_test.logistic_regression(data,file,column)
            else:
                st.write("Invalid test selected")
